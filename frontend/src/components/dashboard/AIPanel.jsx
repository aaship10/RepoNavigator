...
import { X, AlertTriangle, ArrowDownRight, ArrowUpRight, Zap, Brain, Shield, Loader2, RefreshCw, TrendingUp } from 'lucide-react';
...
function ImpactScore({ score }) {
  const getColor = (s) => {
    if (s >= 5) return { color: '#F87171', glow: 'rgba(248,113,113,0.25)', label: 'High Impact' };
    if (s >= 2) return { color: '#FBBF24', glow: 'rgba(251,191,36,0.25)', label: 'Moderate' };
    return { color: '#60A5FA', glow: 'rgba(96,165,250,0.25)', label: 'Low Impact' };
  };
  const { color, glow, label } = getColor(score);
    ...
    >
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="flex flex-col gap-1 pb-1">
          <span className="text-[10px] text-gray-600 leading-tight">files import</span>
          <span className="text-[10px] text-gray-600 leading-tight">this module</span>
        </div>

        {/* Mini bar chart — one pip per importer, up to 10 */}
        <div className="flex items-end gap-0.5 ml-auto pb-0.5">
          {Array.from({ length: Math.min(score, 10) }).map((_, i) => (
            <motion.div
              key={i}
              initial={{ height: 0 }}
              animate={{ height: `${8 + Math.random() * 16}px` }}
              transition={{ delay: 0.55 + i * 0.04, duration: 0.35, ease: 'easeOut' }}
              className="w-1.5 rounded-sm"
              style={{ background: color, opacity: 0.5 + (i / Math.max(score, 1)) * 0.5 }}
            />
          ))}
          {score > 10 && (
            <span className="text-[9px] font-mono ml-1 pb-0.5" style={{ color, opacity: 0.6 }}>+{score - 10}</span>
          )}
        </div>
      </div>
    </motion.div>
...
export default function AIPanel({ selectedNode, repoId, onClose }) {
  ...
  // Extract impact score for the selected node from graph data
  const nodes = data?.graph?.nodes || [];
  const selectedNodeData = nodes.find(n => n.id === selectedNode);
  const impactScore = selectedNodeData?.data?.impactScore ?? null;
      ...
      {selectedNode && data ? (
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-5">
            ...
            {/* Impact Score — shown above Key Functions */}
            {impactScore !== null && (
